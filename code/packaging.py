...
def parse_packaging(packaging_data: str) -> list[dict]:
    ...
    '''
    package = []
    for line in packaging_data.split('/'):
        product = line.split(" in ")[0]
        count = int(product.split()[0])
        product = product.split()[1].strip()
        package.append({product: count})

    product = line.split(" in ")[-1]
    count = int(product.split()[0])
    product = product.split()[1].strip()
    package.append({product: count})
